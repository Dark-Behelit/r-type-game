...
class Game {
    constructor() {
        ...
        this.audioMessage = document.createElement('div');
        this.audioMessage.style.position = 'absolute';
        this.audioMessage.style.top = '50%';
        this.audioMessage.style.left = '50%';
        this.audioMessage.style.transform = 'translate(-50%, -50%)';
        this.audioMessage.style.backgroundColor = 'rgba(0, 0, 0, 0.8)';
        this.audioMessage.style.color = 'white';
        this.audioMessage.style.padding = '20px';
        this.audioMessage.style.borderRadius = '10px';
        this.audioMessage.style.cursor = 'pointer';
        this.audioMessage.style.zIndex = '1000';
        this.audioMessage.innerHTML = 'Haz clic para iniciar el audio';
        document.body.appendChild(this.audioMessage);
    ...
    async init() {
        ...
        const initAudio = async () => {
            try {
                if (!this.audioInitialized) {
                    ...
                    this.bossMusic = new Audio('assets/sounds/boss-music.mp3');
                    this.bossMusic.loop = true;
                    this.bossMusic.volume = 0;
                    await this.bossMusic.play();
                    this.bossMusic.pause();
                    
                    this.audioInitialized = true;
                    this.audioMessage.style.display = 'none';
        ...
        // Añadir evento de clic para iniciar el audio
        this.audioMessage.addEventListener('click', initAudio);
    ...
    fadeOutMusic(duration = 1000, musicTrack = this.backgroundMusic) {
        if (!musicTrack) return Promise.resolve();
        
        return new Promise((resolve) => {
            const originalVolume = musicTrack.volume;
            const steps = 20;
            ...
            const fadeInterval = setInterval(() => {
                ...
                musicTrack.volume = Math.max(0, originalVolume - (volumeStep * currentStep));
                currentStep++;
            }, stepTime);
        });
    }

    fadeInMusic(duration = 1000, musicTrack = this.backgroundMusic) {
        if (!musicTrack) return Promise.resolve();
        ...
        return new Promise((resolve) => {
            ...
            const fadeInterval = setInterval(() => {
                ...
                musicTrack.volume = Math.min(this.musicVolume, volumeStep * currentStep);
                currentStep++;
            }, stepTime);
        });
    }
    ...
    update() {
        ...
        const timeUntilBoss = 60000 - (Date.now() - this.levelStartTime);
        if (!this.bossSpawned && timeUntilBoss <= 3000 && timeUntilBoss > 2900) {
            // Iniciar música 3 segundos antes
            this.startBossMusic();
        }
    ...
    startBossMusic() {
        ...
        if (this.backgroundMusic && this.bossMusic) {
            this.fadeOutMusic(1000, this.backgroundMusic).then(() => {
                this.backgroundMusic.pause();
                this.bossMusic.currentTime = 0;
                this.bossMusic.volume = this.musicVolume * 2.5; // Aumentar el volumen un 150% más que antes
                this.bossMusic.play().catch(e => console.warn('Error al reproducir música de jefe:', e));
                this.fadeInMusic(1000, this.bossMusic);
            });
        }
    }
    ...
    stopBossMusic() {
        ...
        if (this.backgroundMusic && this.bossMusic) {
            this.fadeOutMusic(1000, this.bossMusic).then(() => {
                this.bossMusic.pause();
                this.backgroundMusic.currentTime = 0;
                this.backgroundMusic.volume = this.musicVolume; // Restaurar volumen normal
                this.backgroundMusic.play().catch(e => console.warn('Error al reproducir música de fondo:', e));
                this.fadeInMusic(1000, this.backgroundMusic);
            });
        }
    }
}
...
class BossFight extends Enemy {
    constructor(x, y, level = 1) {
        ...
        // Nueva fórmula de vida que escala exponencialmente
        // Nivel 1: 600 de vida
        // Nivel 2: ~900 de vida
        // Nivel 3: ~1350 de vida
        // Nivel 4: ~2025 de vida
        // Nivel 5: ~3037 de vida
        const baseHealth = 600;
        const healthMultiplier = Math.pow(1.5, level - 1);
        this.health = Math.floor(baseHealth * healthMultiplier);
        this.maxHealth = this.health;
        
        // Ajustar puntos según la dificultad
        this.points = 5000 * Math.pow(1.5, level - 1);
        this.speed = 1 + (level - 1) * 0.1;
        this.shootProbability = 0.03 + (level - 1) * 0.005;
        
        this.movementPhase = 0;
        this.lastPhaseChange = Date.now();
        this.phaseInterval = Math.max(4000 - (level - 1) * 300, 2500);
        this.rightLimit = x - 100;
        this.shootPattern = 0;
        this.isDead = false;
        this.initialX = x - 100;

        // Iniciar música de jefe
        if (window.game) {
    ...
    shoot(bulletArray) {
        ...
        switch(this.shootPattern) {
            ...
            case 1:
                ...
                break;
            default:
                // Patrón circular con más balas según el nivel
    ...
    takeDamage(damage, isSuperShot = false) {
        ...
        if (this.health <= 0 && !this.isDead) {
            ...
            // Detener música de jefe y volver a la música normal
            if (window.game) {
                window.game.stopBossMusic();
            }
