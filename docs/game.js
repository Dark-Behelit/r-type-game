class Resources {
    static async load() {
        ...
        try {
            // Si existe statusElement, actualizamos su texto
            if (statusElement) {
            ...
            // Si hay un elemento de carga en index.html, también lo ocultamos
            ...
            // Si hay un elemento de carga en index.html, mostramos el error ahí también
            const loadingElement = document.getElementById('loading');
            if (loadingElement) {
                loadingElement.textContent = 'Error generando recursos. Por favor, recarga la página.';
            }
...
class Game {
    ...
    showGameOver() {
        ...
        if (isHighScore) {
            // Si es puntuación alta, mostrar formulario para introducir nombre
            this.showNameInputForm(() => this.displayGameOverScreen());
        } else {
            // Si no es puntuación alta, mostrar directamente la pantalla de Game Over
            this.displayGameOverScreen();
        }
    }
    
    // Mostrar la pantalla de Game Over con la tabla de puntuaciones
    displayGameOverScreen() {
        ...
        gameOverScreen.style.maxHeight = '80vh';
        gameOverScreen.style.overflowY = 'auto';
        gameOverScreen.innerHTML = `
            <h2>¡Juego Terminado!</h2>
            <p>Tu puntuación: ${this.score}</p>
            <button id="restartButton" style="font-size: 18px; padding: 8px 15px; margin: 10px; background-color: #4CAF50; color: white; border: none; border-radius: 5px; cursor: pointer;">Reiniciar</button>
    ...
    showVictoryScreen() {
        ...
        if (isHighScore) {
            // Si es puntuación alta, mostrar formulario para introducir nombre
            this.showNameInputForm(() => this.displayVictoryScreen());
        } else {
            // Si no es puntuación alta, mostrar directamente la pantalla de Victoria
            this.displayVictoryScreen();
        }
    }
    
    // Mostrar la pantalla de Victoria con la tabla de puntuaciones
    displayVictoryScreen() {
        ...
        victoryScreen.style.maxHeight = '80vh';
        victoryScreen.style.overflowY = 'auto';
        victoryScreen.innerHTML = `
            <h2>¡Victoria!</h2>
            <p>¡Has completado todos los niveles!</p>
            <p>Tu puntuación final: ${this.score}</p>
            <button id="restartButton" style="font-size: 18px; padding: 8px 15px; margin: 10px; background-color: #4CAF50; color: white; border: none; border-radius: 5px; cursor: pointer;">Jugar de nuevo</button>
            <div style="margin-top: 20px;">
                ${this.createHighScoresTable()}
            </div>
        `;
    ...
    loadHighScores() {
        const savedScores = localStorage.getItem('rtypeHighScores');
        return savedScores ? JSON.parse(savedScores) : [];
    }
    
    // Guardar puntuaciones altas en localStorage
    saveHighScores() {
        localStorage.setItem('rtypeHighScores', JSON.stringify(this.highScores));
    }
    
    // Comprobar si una puntuación merece estar en la tabla
    isHighScore(score) {
        if (this.highScores.length < 20) return true;
        return score > this.highScores[this.highScores.length - 1].score;
    }
    
    // Añadir una nueva puntuación alta
    addHighScore(name, score) {
        const newScore = { name, score, date: new Date().toLocaleDateString() };
        this.highScores.push(newScore);
        this.highScores.sort((a, b) => b.score - a.score); // Ordenar de mayor a menor
        if (this.highScores.length > 20) {
            this.highScores = this.highScores.slice(0, 20); // Mantener solo las 20 mejores
        }
        this.saveHighScores();
    }
    
    // Crear la tabla HTML de puntuaciones altas
    ...
    // Mostrar formulario para introducir nombre
    showNameInputForm(callback) {
        ...
        inputForm.style.fontSize = '24px';
        inputForm.style.backgroundColor = 'rgba(0, 0, 0, 0.9)';
        inputForm.style.padding = '20px';
        inputForm.style.borderRadius = '10px';
        inputForm.style.zIndex = '2000';
        inputForm.innerHTML = `
            <h3>¡Nueva Puntuación Alta!</h3>
            <p>Tu puntuación: ${this.score}</p>
            <p>Introduce tu nombre:</p>
            <input type="text" id="playerName" maxlength="15" style="font-size: 18px; padding: 5px; width: 200px; margin-bottom: 15px;">
            <br>
            <button id="submitScore" style="font-size: 18px; padding: 8px 15px; background-color: #4CAF50; color: white; border: none; border-radius: 5px; cursor: pointer;">Guardar</button>
        `;
        ...
        document.getElementById('submitScore').addEventListener('click', () => {
            ...
            this.addHighScore(playerName, this.score);
            callback();
        });
    }
}
...
window.onload = async () => {
    try {
        ...
        // Mostrar el error en resourceStatus si existe
        const statusElement = document.getElementById('resourceStatus');
        if (statusElement) {
            statusElement.textContent = 'Error al iniciar el juego. Por favor, recarga la página.';
        }
        
        // Mostrar el error en loading si existe (para index.html)
        const loadingElement = document.getElementById('loading');
        if (loadingElement) {
            loadingElement.textContent = 'Error al iniciar el juego. Por favor, recarga la página.';
        }
    }
}; 